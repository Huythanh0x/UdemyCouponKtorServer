...
sudo docker run --name ktor-mysql --network=ktor-mysql-network -p 3306:3306 -e MYSQL_ROOT_PASSWORD=password -d  mysql:latest
sudo docker  run --name udemy_coupon_ktor_server --network=ktor-mysql-network -p 8080:8080  --rm  -d huythanh0x/udemy_coupon_ktor_server:latest
